...
export async function fetchOrderDetailAction(id: string) {
  try {
    const cookieStore = await cookies();
    const token = cookieStore.get('halcon_token')?.value;
    ...
    console.error("Fetch Order Detail Action Error:", error);
    return { success: false, message: "Error fetching order detail: " + (error.message || String(error)) };
  }
}
